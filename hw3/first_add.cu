...
__global__ void
kernel3(dtype *g_idata, dtype *g_odata, unsigned int n) // n is the number of elements we have to work on; we have n/2 threads here
{
  ...
  if(i < (n >> 1) ) {
    scratch[threadIdx.x] = g_idata[i] + g_idata[i + (n >> 1) ]; // adding and loading
  } else {
...
main(int argc, char** argv)
{
	...
	/* naive kernel */
	whichKernel = 3; // this is taking care of spawning two threads per element
