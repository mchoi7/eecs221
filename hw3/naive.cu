...
kernel0 (dtype *input, dtype *output, unsigned int n)
{
  ...
  if(threadIdx.x == 0) {
    output[bid] = scratch[0]; // the blocks overwrite the first "numOfBlocks" elements in the output array. each block writes at the block idx location. 
  }
}
...
main(int argc, char** argv)
{
  ...
  /* ================================================== */
  /* GPU kernel */
  dim3 gb(16, ((blocks + 16 - 1) / 16), 1);   // (gridDim.x, gridDim.y, 1)
  dim3 tb(threads, 1, 1);                     // (blockDim.x, blockDim.y, blockDim.z)

  /* warm up */
  kernel0 <<<gb, tb>>> (d_idata, d_odata, N); // gb blocks, tb threads each
  ...
  while(s > 1) {
    ...
    s = (s + threads - 1) / threads; // updating the number of results we have from this stage (on which the next reduction needs to occur)
  }
  cudaThreadSynchronize ();
